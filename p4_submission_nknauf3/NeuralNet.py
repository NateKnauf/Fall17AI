...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        """YOUR CODE"""
        inActs = [1] + inActs
        for i, (act, weight) in enumerate(zip(inActs, self.weights)):
            mod = alpha * delta * act
            self.weights[i] += mod
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            for node in layer:
                thisLayer = thisLayer + [node.sigmoidActivation(lastLayer)]
            lastLayer = thisLayer
            outLists.append(thisLayer)
            thisLayer = []
        return outLists
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for nextNeuronNum in xrange(len(nextLayer)):
                        weightedSum += nextLayer[nextNeuronNum].weights[neuronNum + 1] * deltas[0][nextNeuronNum]
                    delta = gPrime * weightedSum
                    hiddenDelta.append(delta)
                    # delta = gPrime * sum([nL.weights[neuronNum + 1] * deltas[0][i] for i, nL in enumerate(self.layers[layerNum + 1])])
                    # hiddenDelta.append(delta)
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None, printer=True):
    """
    ...
    while weightChange > weightChangeThreshold and iteration < maxItr:
        error, weightChange = nnet.backPropLearning(examplesTrain, alpha)
    ...
    for example in examplesTrain:
        ...
        calculated = []
        total += 1
        for x in output[len(output) - 1]:
            calculated.append(round(x))
        if calculated == example[1]:
            totalCorrect += 1
    trainError = (total - totalCorrect) / total * 100
    ...
    for example in examplesTest:
        ...
        if calculated == example[1]:
            testCorrect += 1
    testError = (total - testCorrect)
    testAccuracy = 1.0 * testCorrect / total
    if printer:
