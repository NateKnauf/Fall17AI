...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        if len(corners) == 1 and current in corners:
            return True
        return False
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if not self.walls[nextx][nexty]:
                if current in corners:
                    successors.append((((nextx, nexty), [nc for nc in corners if nc != current]), action, 1))
                else:
...
def cornersHeuristic(state, problem):
    ...
    current, scorners = state
    return max( [ (abs(current[0]-c[0])+abs(current[1]-c[1])) for c in scorners] )
...
def foodHeuristic(state, problem):
    ...
    for x, row in enumerate(foodGrid):
        for y, cell in enumerate(row):
            if foodGrid[x][y]:
                total.append(abs(px-x)+abs(py-y))
    return max(total)
    ...
    def findPathToClosestDot(self, gameState):
        ...
        for x, row in enumerate(food):
            for y, col in enumerate(row):
                if food[x][y]:
                    ds.append(abs(startPosition[0]-x)+abs(startPosition[1]-y))
                    ps.append((x, y))
        if len(ds) > 0:
            return search.bfs(PositionSearchProblem(gameState, start=startPosition, goal=ps[ds.index(min(ds))]))
        return []
