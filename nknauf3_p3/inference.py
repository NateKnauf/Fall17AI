        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            ...
            for newPos, p in newPosDist.items():
                allPossible[newPos] += p*self.beliefs[oldPos]
        allPossible.normalize()
        self.beliefs = allPossible
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for pos in self.legalPositions:
                newBeliefs[pos] += emissionModel[util.manhattanDistance(pos, pacmanPosition)] * oldBeliefs[pos]
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for index, particle in enumerate(self.particles):
            temp = 1
            for i in range(self.numGhosts):
                if noisyDistances[i] == None: # jailed
        ...
        if not any(beliefs.values()): # restart
            self.initializeParticles()
            for index, particle in enumerate(self.particles):
                for i in range(self.numGhosts):
                    if noisyDistances[i] == None: # jailed
