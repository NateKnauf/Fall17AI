...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for belief in livingGhostPositionDistributions:
            ghostTargets.append(belief.argMax())

        ghostP = 0
        targetPos = None
        # Finds the goal probability
        for i, ghostPos in enumerate(ghostTargets):
            if livingGhostPositionDistributions[i][ghostPos] >= ghostP:
                ghostP = livingGhostPositionDistributions[i][ghostPos]
                targetPos = ghostPos

        nextMove = None
        targetDis = None
        for move in legal:
            nextLoc = Actions.getSuccessor(pacmanPosition, move)
            nextDis = self.distancer.getDistance(nextLoc, targetPos)
            if targetDis == None or targetDis > nextDis:
                targetDis = nextDis
                nextMove = move
        return nextMove
